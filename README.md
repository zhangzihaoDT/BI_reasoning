...
- Agent: [`agents/query_agent_reasoner.py`](agents/query_agent_reasoner.py)
- Pipeline: [`pipelines/simple_query_reasoner.py`](pipelines/simple_query_reasoner.py)

### 核心优化点对比

| 维度           | 原版 (`query_agent.py`)                | Reasoner 版 (`query_agent_reasoner.py`)                              |
| :------------- | :------------------------------------- | :------------------------------------------------------------------- |
| **模型与模式** | `deepseek-chat`<br>单轮提示生成        | **`deepseek-reasoner` (Thinking Mode)**<br>多轮思考-工具调用链路     |
| **工具调用**   | 依赖正则/启发式规则提取 JSON，手动执行 | **原生 Tool Calls**<br>定义标准 JSON Schema，模型自主决策调用        |
| **思考过程**   | 无显式思考，容易幻觉                   | **连续思考链**<br>多轮对话中传递 `reasoning_content`，确保上下文一致 |
| **输出形态**   | 返回结构化数据或单值                   | **自然语言洞察**<br>直接输出包含数据支撑的业务结论                   |
| **灵活性**     | 复杂查询（如多维聚合）解析准确率低     | **高鲁棒性**<br>自动识别维度、过滤器，支持模糊意图                   |

### 运行效果示例

**Case: "LS6 和 LS9 昨天的锁单对比"**

**1. 原版 (`simple_query.py`)** - _仅返回原始数据_

```text
--- Final Result ---
🔢 锁单量 | series | rows=2
{"series": "LS6", "value": 172}
{"series": "LS9", "value": 42}
```

**2. Reasoner 版 (`simple_query_reasoner.py`)** - _自动计算占比与倍数，输出自然语言报告_

```text
🚀 Starting Reasoner Query Pipeline...

--- Final Result ---
昨天（2026‑01‑11），LS6 与 LS9 的锁单数据对比如下：
...
**对比分析**：
- LS6 的锁单量是 LS9 的 **4.1 倍**（172 ÷ 42 ≈ 4.095）。
- 在两者合计的 214 个锁单中，LS6 占比 **80.4%**，LS9 占比 **19.6%**。
```

### 10.2 每日经营分析升级 (Pipeline Upgrade)
